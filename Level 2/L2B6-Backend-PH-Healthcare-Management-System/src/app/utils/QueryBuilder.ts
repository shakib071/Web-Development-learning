...
> {
    ...
    filter() : this {
        ...
        Object.keys(filterParams).forEach((key) => {
            ...
            // !filterableFields — if no filterableFields configured at all → allow everything. Developer didn't restrict anything.
            // filterableFields.length === 0 — if empty array configured → allow everything. Same as above.
            // filterableFields.includes(key) — if this specific key is in the allowed list → allow it.
    ...
    fields() : this {
        ...
        if(fieldsParam && typeof fieldsParam === 'string'){
            ...
            //Why? — Prisma throws an error if BOTH select and include exist in the same query:
    ...
    // dynamicInclude() — User Controls Via URL ?include=reviews,appointments
    dynamicInclude(
        includeConfig : Record<string, unknown>,
        defaultInclude ?: string[]
    ) : this{
        // includeConfig is the whitelist — it defines which relations are allowed to be included.
        if(this.selectFields){
            return this;
        }
    ...
    private parseFilterValue(value : unknown) : unknown {
        ...
        //Number("")  →  0   →  isNaN(0) = false  →  would pass without this check!
        if(typeof value === 'string' && !isNaN(Number(value)) && value != ""){
            return Number(value);
        }
        // URL: ?gender=MALE&gender=FEMALE
        // Express parses as: { gender: ["MALE", "FEMALE"] }
    ...
    private parseRangeFilter(value : Record<string, string | number>) : PrismaNumberFilter | PrismaStringFilter | Record<string, unknown> {
        ...
        Object.keys(value).forEach((operator) => {
            ...
            // URL: ?id[in][]=1&id[in][]=2&id[in][]=3
            // Express parses as: { id: { in: ["1", "2", "3"] } }
            switch(operator){
                case 'lt':
                case 'lte':
                case 'gt':
